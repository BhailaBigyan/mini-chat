...
// MongoDB connection
const MONGO_URI = 'mongodb+srv://applebanana9862:dbminichat@mini-chat.r4hplqs.mongodb.net/?retryWrites=true&w=majority&appName=mini-chat';
...
app.get('/chat', isAuth, (req, res) => res.sendFile(__dirname + '/public/chat.html'));
app.get('/chat-room', isAuth, (req, res) => res.sendFile(__dirname + '/public/chat-room.html'));
app.get('/private/:room', isAuth, (req, res) => res.sendFile(__dirname + '/public/private.html'));
app.get('/join', (req, res) => {  res.sendFile(path.join(__dirname, 'public/join.html'));});
app.get('/delete-room', isAuth, (req, res) => res.sendFile(__dirname + '/public/delete-room.html'));
...
app.get('/api/private-rooms', (req, res) => res.json(Object.keys(privateRooms)));
...
io.on('connection', socket => {
 ...
 socket.on('new user', async (userData) => {
  ...
  const history = await Message.find({ room: 'public' }).sort({ timestamp: 1 }).limit(100);
  socket.emit('chat history', history);
  socket.to('public').emit('system message', `${socket.full_name} joined the chat`);
...
socket.on('chat message', async (messageText) => {
  ...
  const room = Array.from(socket.rooms).find(r => r !== socket.id) || 'public';

  if (!username || !messageText) return;

  const msg = await Message.create({
    username: full_name,
    message: messageText,  // <- ensure this is a string
    room
  });
...
socket.on('join private', async ({ room, username }) => {
  ...
  if (existingRoom) {
    ...
    if (existingRoom.nickname === existingRoom.owner || existingRoom.nickname === '') {
  ...
  socket.emit('chat history', history);
  socket.to(room).emit('system message', `${socket.full_name} joined private room`);
});
